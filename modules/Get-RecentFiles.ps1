try {
    ...
    if (Test-Path $recentDir) {
        ...
        if ($recentFiles) {
            Write-Host "    [+] Recent files found." -ForegroundColor Green
        ...
        Write-Host "    [!] Recent directory not found." -ForegroundColor Red
        # Log to Markdown file
        "## Recent Files`n" | Out-File -Append -FilePath "$logDir/RecentFiles.md"
    ...
    Write-Host "    [!] Unable to retrieve recent files: $_" -ForegroundColor Red
    # Log to Markdown file
    "## Recent Files`n" | Out-File -Append -FilePath "$logDir/RecentFiles.md"
